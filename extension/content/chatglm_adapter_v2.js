(() => {
  ...
  function roleOf(node) {
    ...
    for (const attr of attrs) {
      ...
      if (/(assistant|ai|model|bot|助手|模型|glm|清言)/.test(lower)) return 'assistant';
    ...
    if (/助手|assistant|清言|模型|ai|glm/.test(aria)) return 'assistant';

    const cls = (node.className || '').toString().toLowerCase();
    if (/(user|human|mine)/.test(cls)) return 'user';
    if (/(assistant|ai|bot|model|glm|清言)/.test(cls)) return 'assistant';
  ...
  function sendSnapshot() {
    ...
    adapter.debounceObserve({ platform: 'chatglm', messages });
  }
