(() => {
  ...
  function roleOf(node) {
    ...
    for (const attr of attrs) {
      ...
      if (/(assistant|ai|model|bot|助手|模型|kimi)/.test(lower)) return 'assistant';
    ...
    if (/助手|assistant|kimi|模型|ai/.test(aria)) return 'assistant';

    const cls = (node.className || '').toString().toLowerCase();
    if (/(user|human|mine)/.test(cls)) return 'user';
    if (/(assistant|ai|bot|model|kimi)/.test(cls)) return 'assistant';
  ...
  function sendSnapshot() {
    ...
    adapter.debounceObserve({ platform: 'kimi', messages });
  }
