(() => {
  ...
  function roleOf(node) {
    ...
    for (const attr of attrs) {
      ...
      if (/(assistant|ai|model|bot|助手|模型|trae)/.test(lower)) return 'assistant';
      if (/(system)/.test(lower)) return 'system';
      if (/(tool|function)/.test(lower)) return 'tool';
    }
    const aria = (node.getAttribute('aria-label') || '').toLowerCase();
    if (/用户|user|我/.test(aria)) return 'user';
    if (/助手|assistant|trae|模型|ai/.test(aria)) return 'assistant';
    const cls = (node.className || '').toString().toLowerCase();
    if (/(user|human|mine)/.test(cls)) return 'user';
    if (/(assistant|ai|bot|model|trae)/.test(cls)) return 'assistant';
    return '';
  }
  ...
  function sendSnapshot() {
    ...
    if (!query) return;
    adapter.debounceObserve({ platform: 'trea', messages });
  }
